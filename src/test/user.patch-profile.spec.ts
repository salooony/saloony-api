...
describe('PatchUserProfileUseCase', () => {
  ...
  it('throws conflict when updating to an existing mobile number owned by another user', async () => {
    const user1 = await userRepository.save(createUserEntity('user5@email.com', '+21655555111'));
    await userRepository.save(createUserEntity('user6@email.com', '+21666666111'));

    const request = plainToInstance(PatchUserProfileRequestDto, {
      mobileNumber: '+21666666111',
    });
